...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        "*** YOUR CODE HERE ***"
        #initally empty list which represents the visited corners
        self.visitedCorners = []
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        #start state will contain the starting location and the list of visited corners
        #if starting point is a corner then size of visited corners is 1
        #otherwise size of visited corners is 0
        ...
        visitedC = []
        visitedC = self.visitedCorners

        if (self.corners.__contains__(self.startingPosition)):
            visitedC.append(self.startingPosition)
            start.append(visitedC)
            return start
        else:
            start.append(visitedC)
            return start
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        #if size of the visited corners list is 4 then it means we visited all the corners and reached to goal state

        numVisitedCorners = len(state[1])

        if (numVisitedCorners == 4):
            return True
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            visitedCorners = []
            visitedCorners.extend(state[1])

            if (self.walls[nextx2][nexty2] == False):
                for corner in self.corners:
                    if (corner == (nextx2, nexty2) and visitedCorners.__contains__(corner) == False):
                        visitedCorners.append(corner)
                    state0 = []
                    state0.append((nextx2, nexty2))
                    state0.append(visitedCorners)
                    res = []
                    res.append(state0)
                    res.append(action)
                    res.append(1)
...
def cornersHeuristic(state, problem):
    ...
    if (len(state) > 1):
        ...
        for n in corners:
            if (visitedCorners.__contains__(n) == False):
                realCorners.append(n)

    for c in realCorners:
    ...
    return sorted(distances)[3-len(visitedCorners)]
...
def foodHeuristic(state, problem):
    ...
    return sorted(distances)[len(distances)-1]
